      ...
      <div className="bg-gradient-to-br from-slate-900/50 via-purple-900/30 to-blue-900/50">
        ...
        <main className="relative z-10 px-4 sm:px-6 lg:px-8 pb-20">
          ...
          <div className="text-center space-y-8 max-w-4xl mx-auto py-16">
            <h2 className="text-3xl sm:text-5xl font-bold bg-gradient-to-r from-indigo-400 via-purple-400 to-indigo-400 bg-clip-text text-transparent">
              Projenize Başlayın
            </h2>
            <button onClick={() => setShowWhatsAppModal(true)} className="px-10 py-5 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full text-white font-bold text-lg transition-all duration-300 hover:from-indigo-700 hover:to-purple-700 hover:scale-105">
              WhatsApp ile İletişim
            </button>
          </div>
        </main>

        <button onClick={() => setShowWhatsAppModal(true)} className="fixed bottom-6 right-6 z-50 w-16 h-16 bg-green-500 hover:bg-green-600 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center">
        ...
        {showWhatsAppModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-gradient-to-br from-slate-900/90 via-purple-900/60 to-blue-900/80 rounded-2xl p-6 max-w-sm w-full mx-4">
              <button onClick={() => setShowWhatsAppModal(false)} className="float-right text-slate-400">×</button>
              <h3 className="text-xl font-semibold text-white mb-6">WhatsApp İletişim</h3>
              <div className="space-y-3">
                <a href="https://wa.me/905398884561" target="_blank" rel="noopener noreferrer" className="block p-4 bg-green-600/20 rounded-xl text-white text-center" onClick={() => setShowWhatsAppModal(false)}>0(539) 888 45 61</a>
                <a href="https://wa.me/905510670094" target="_blank" rel="noopener noreferrer" className="block p-4 bg-green-600/20 rounded-xl text-white text-center" onClick={() => setShowWhatsAppModal(false)}>0(551) 067 00 94</a>
              </div>
            </div>
          </div>
        )}
